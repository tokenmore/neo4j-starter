...
(function () {
    var W = {
        ...
        node: {node_scale: 1, node_size: 40, node_color: "black", node_opacity: "1"},
    ...
    function n(t, e, n, M) {
        ...
        var r = J;
        var i = {nodes: [], links: []};
        i = JSON.parse(JSON.stringify(typeof t !== "undefined" ? t : i));
        var a = d3.forceSimulation().alpha(0.8).alphaDecay(.002).alphaMin(.002).force("r", null).force("center", d3.forceCenter(typeof arguments[2] !== "undefined" ? n : K.width / 2, typeof arguments[3] !== "undefined" ? M : K.height / 2)).force("link", d3.forceLink().id(function (t) {
            return t.id
        }).strength(W.graph.gravitation)).force("charge", d3.forceManyBody().strength(W.graph.repulsion)).force("collision", d3.forceCollide(W.node.node_size)).on("tick", l).on("end", function () {
            U()
        });
        var o = d3.forceSimulation().alpha(0.8).alphaDecay(.01).alphaMin(.002).force("r", d3.forceRadial(100, typeof arguments[2] !== "undefined" ? n : K.width / 2, typeof arguments[3] !== "undefined" ? M : K.height / 2)).force("center", d3.forceCenter(typeof arguments[2] !== "undefined" ? n : K.width / 2, typeof arguments[3] !== "undefined" ? M : K.height / 2)).force("link", d3.forceLink().id(function (t) {
            return t.id
        }).strength(0)).force("collision", d3.forceCollide(W.node.node_size)).on("tick", l).on("end", function () {
            U()
        });
        var s = F.append("g").attr("id", "network_graph").style("user-select", "none").style("font-size", 7).style("text-anchor", "middle");
        V.call(d3.zoom().scaleExtent([.1, 10]).on("zoom", function () {
            F.attr("transform", d3.event.transform)
        })).on("dblclick.zoom", null);
        s.append("g").attr("id", "temp_layout");
        s.append("g").attr("id", "link_layout");
        s.append("g").attr("id", "node_layout");
        var u = s.append("g").attr("id", "defs_layout");
        u.append("pattern").attr("id", "pattern-0").append("image").attr("xlink:href", "https://www.easyicon.net/api/resizeApi.php?id=1202871&size=128");
        u.append("pattern").attr("id", "pattern-1").append("image").attr("xlink:href", q);
        u.append("pattern").attr("id", "pattern-2").append("image").attr("xlink:href", "https://www.easyicon.net/api/resizeApi.php?id=1158148&size=128");
        u.selectAll("pattern").attr("width", "100%").attr("height", "100%").select("image").attr("width", 20).attr("height", 20);
        var j = [];
        var c = [];
        Y(i, e);
        ...
        function l() {
            ...
            c.select(".link-path").attr("d", function (t) {
                return E(t, W.link.link_type)
            });
            c.select("text").attr("dx", function (t) {
                return v(t)
            }).attr("transform", function (t) {
                return m(t)
            });
            X.selectAll(".link").select(".link-path").attr("d", function (t) {
                return E(t, W.link.link_type)
            });
            X.selectAll(".link").select("text").attr("dx", function (t) {
                return v(t)
            }).attr("transform", function (t) {
                return m(t)
            })
        ...
        function L(t) {
            j = s.select("#node_layout").selectAll("g").data(t);
            j.exit().remove();
            var e = j.enter().append("g").attr("class", "node").attr("transform", function (t) {
                return "x" in t && "y" in t ? "translate(" + t.x + ", " + t.y + ")" : ""
            }).on("mousedown.select-node", g).on("mouseenter.enter-node", w).on("mouseleave.leave-node", z).on("dblclick", D).call(d3.drag().on("start", S).on("drag", A).on("end", O));
            ...
            j = j.merge(e);
            j.selectAll("circle").attr("r", function (t) {
                if (!("size" in t)) {
                    t["size"] = W.node.node_size
                }
                return t.size * W.node.node_scale
            });
            y()
        }
        ...
        function y() {
            ...
            j.select("text").remove();
            j.filter(function (t) {
                return typeof t.label !== "undefined" ? true : false
            }).append("text").attr("x", function (t) {
                return N(d3.select(this), t[e])
            }).attr("dy", ".35em").style("display", W.show_status.node_text === true ? "block" : "none").style("pointer-events", "none").style("fill", "white").text(function (t) {
                return t.label
            })
        }
        ...
        function N(t, e) {
            ...
            if (n <= 4) {
                ...
                t.text("");
                t.append("tspan").attr("x", 0).attr("y", a).text(function () {
                    return M
                });
                t.append("tspan").attr("x", 0).attr("y", o).text(function () {
                    return r
                });
        ...
        function d(t, e) {
            ...
            n.exit().remove();
            var M = n.enter().append("g").attr("class", "link");
            M.append("path").attr("class", "link-path").style("fill", "none").style("cursor", "pointer").style("stroke", "#6f7071").style("stroke-width", "4px");
            M.append("marker").attr("class", "link-marker").attr("markerUnits", "userSpaceOnUse").attr("viewBox", "0 -50 100 100").attr("refY", 0).attr("markerWidth", 12).attr("markerHeight", 12).attr("orient", "auto").append("path").attr("d", "M20,0 L0,-30 L90,0 L0,30 L20,0").style("fill", "#000000");
            M.append("text").attr("class", "link-text").attr("dx", function (t) {
                return v(t)
            }).attr("dy", 2).attr("transform", function (t) {
                return m(t)
            }).append("textPath").style("fill", "#000000").style("display", W.show_status.link_text === true ? "block" : "none");
            n = n.merge(M);
            ...
            n.select(".link-path").attr("id", function (t) {
                ...
                if (!("width" in t)) {
                    t["width"] = W.link.link_width
                }
                return t.width
            }).on("mousedown.select-link", T).on("mouseover.hover-link", x);
        ...
        function D(M) {
            U();
            var t = (M.size ? M.size : W.node.size) + 8;
            var e = t + 30;
            var n = Math.cos(175 / 360 * Math.PI);
            var r = Math.sin(175 / 360 * Math.PI);
            var i = Math.cos(55 / 360 * Math.PI);
            var a = Math.sin(55 / 360 * Math.PI);
            var o = -(2 * i + 2 * Math.sqrt(3) * a);
            var s = Math.pow(i, 2) + 3 * Math.pow(a, 2) + 2 * Math.sqrt(3) * i * a - 3 * Math.pow(e, 2);
            var u = (Math.sqrt(Math.pow(o, 2) - 16 * s) - o) / 8;
            var c = Math.sqrt(Math.pow(e, 2) - Math.pow(u, 2));
            var l = n;
            var L = Math.sqrt(1 - Math.pow(l, 2));
            var y = "M" + t * n + ", -" + t * r + " " + "A" + t + ", " + t + " 0 0, 1 " + t * i + ", " + t * a + "L" + u + "," + c + " " + "A" + e + ", " + e + " 0 0, 0 " + e * l + ", -" + e * L + "L" + t * n + ",-" + t * r;
            d3.selectAll(".menu").remove();
            var N = d3.select(this).selectAll(".menu").data([0, 1, 2]).enter().append("g").attr("class", "menu");
            N.append("path").attr("id", function (t, e) {
                return "menu-path" + e
            }).attr("d", y).attr("transform", function (t, e) {
                ...
                if (t === 0) {
                    ...
                    V.on("mousemove.add-link", function () {
                        var t = h("#container");
                        var e = t.k;
                        n.attr("x2", (d3.event.x - K.x - t.x) / e).attr("y2", (d3.event.y - K.y - t.y) / e);
                        n.style("opacity", 1)
                    });
                    j.selectAll("circle").style("cursor", "pointer").on("click.add-link", function (t) {
                        n.remove();
                        V.on("mousemove.add-link", null);
                        j.selectAll("circle").style("cursor", "move").on("click.add-link", null);
                        B(e, t, "TEST")
                    })
                } else if (t === 2) {
                    G(M)
            ...
            N.append("rect").attr("class", "menu-icon").attr("id", function (t, e) {
                return "menu-icon-" + e
        ...
        function A(t) {
            ...
            k.select("path").attr("d", function (t) {
                return E(t, W.link.link_type)
            });
            k.select("text").attr("dx", function (t) {
                return v(t)
            }).attr("transform", function (t) {
                return m(t)
            });
        ...
        function E(t, e) {
            if (t.source === t.target) {
                var n = typeof t.source.size === "undefined" ? W.node.node_size : t.source.size;
                var M = t.source.x + n * W.node.node_scale / 4 * 3;
                var r = t.source.y + n * W.node.node_scale / 4;
                var i = t.source.x + n * W.node.node_scale / 4;
                var a = t.source.y + n * W.node.node_scale / 4 * 3;
                var o = M + 150;
                var s = r;
                var u = i;
                var c = a + 150;
                return "M" + M + " " + r + "C" + o + " " + s + ", " + u + " " + c + ", " + i + " " + a
            }
            var l = null;
            var L = t.source.x;
            var y = t.source.y;
            ...
            if (e === "slink") {
                ...
                l = "M " + L + "," + y + " C" + D + "," + f + " " + z + "," + x + " " + p + "," + I + " S" + k + "," + S + " " + N + "," + j
            } else if (e === "hlink") {
                l = "M " + L + "," + y + " L" + p + "," + y + " " + " L" + p + "," + j + " L" + N + "," + j
            } else if (e === "vlink") {
                l = "M " + L + "," + y + " L" + L + "," + I + " " + " L" + N + "," + I + " L" + N + "," + j
            }
            return l
        }
        ...
        function v(t) {
            ...
            var M = t.target.x;
            var r = t.target.y;
            var i = Math.sqrt(Math.pow(M - e, 2) + Math.pow(r - n, 2));
            return i / 2
        }
        ...
        function m(t) {
            ...
            var M = t.target.x;
            var r = t.target.y;
            return e <= M ? "" : "rotate(180, " + (e + M) / 2 + " " + (n + r) / 2 + ")"
        }
        ...
        function h(t) {
            ...
            var r = n && n[2] || 1;
            var i = e && /rotate/.test(e) && e.match(/\s?rotate\(([^\)]+)/);
            var a = i && i[1] || 0;
            var o = M[0];
            var s = M[1];
            return {x: o, y: s, k: r, rotate: a}
        }

        function Y(t) {
            i = t;
            if (e === "force") {
                a.nodes(t.nodes).force("link").links(t.links).distance(200)
            } else if (e === "radius") {
                o.nodes(t.nodes).force("link").links(t.links)
            }
            L(t.nodes);
            d(t.links, s.select("#link_layout"));
            b()
        }
        ...
        function b() {
            W.graph.calculating = true;
            d3.select("#network-status").style("animation", "calc ease 1s infinite");
            a.alpha(0.7).restart()
        }

        function U() {
            W.graph.calculating = false;
            d3.select("#network-status").style("animation", "none");
            a.stop()
        }

        function P(t) {
            W.graph.layout_mode = t
        }

        function Z(t) {
            W.graph.analyse_mode = t
        }

        function _(t, e, n) {
            U();
            t["x"] = arguments[1] ? e : K.width / 2;
            t["y"] = arguments[2] ? n : K.height / 2;
            i.nodes.push(t);
            Y(i)
        }
        ...
        function B(e, n, t) {
            ...
            i.links.push({source: M, target: r, type: t});
            Y(i)
        }
        ...
        function G(e) {
            ...
            i.nodes.splice(e.index, 1);
            i.links = t.links.filter(function (t) {
    ...
    var V = null, F = null, X = null;
    var K = Object();

    function M(t) {
        V = d3.select("#" + (typeof t !== "undefined" ? t : "body"));
        V.on("click", function () {
            d3.selectAll(".menu").remove()
        });
        F = V.append("g").attr("id", "container");
        X = F.append("g").attr("id", "connect_layout");
        K = document.getElementById(t).getBoundingClientRect();
