...
(function () {
    var R = {
        graph: {layout_mode: 0, analyse_mode: false, gravitation: .03, repulsion: -400, calculate_state: true},
    ...
    function n(t, e, n, M) {
        ...
        var r = d3.forceSimulation().alpha(1).alphaDecay(.002).alphaMin(.002).force("r", null).force("center", d3.forceCenter(typeof arguments[2] !== "undefined" ? n : q.width / 2, typeof arguments[3] !== "undefined" ? M : q.height / 2)).force("link", d3.forceLink().id(function (t) {
            return t.id
        }).strength(R.graph.gravitation)).force("charge", d3.forceManyBody().strength(R.graph.repulsion)).force("collision", d3.forceCollide(R.node.node_size)).on("tick", L).on("end", function () {
            b()
        });
        var o = d3.forceSimulation().alpha(1).alphaDecay(.01).alphaMin(.002).force("r", d3.forceRadial(100, typeof arguments[2] !== "undefined" ? n : q.width / 2, typeof arguments[3] !== "undefined" ? M : q.height / 2)).force("center", d3.forceCenter(typeof arguments[2] !== "undefined" ? n : q.width / 2, typeof arguments[3] !== "undefined" ? M : q.height / 2)).force("link", d3.forceLink().id(function (t) {
            return t.id
        }).strength(0)).force("collision", d3.forceCollide(R.node.node_size)).on("tick", L).on("end", function () {
            b()
        });
        var s = V.append("g").attr("id", "network_graph").style("user-select", "none").style("font-size", 7).style("text-anchor", "middle");
        J.call(d3.zoom().scaleExtent([.1, 10]).on("zoom", function () {
            V.attr("transform", d3.event.transform)
        ...
        u.append("pattern").attr("id", "pattern-0").append("image").attr("xlink:href", K);
        u.append("pattern").attr("id", "pattern-1").append("image").attr("xlink:href", $);
        u.append("pattern").attr("id", "pattern-2").append("image").attr("xlink:href", tt);
        ...
        function L() {
            ...
            if (X.length !== 0) {
                X.select(".link-path").attr("d", function (t) {
                    return C(t)
                });
        ...
        function l(t) {
            y = s.select("#node_layout").selectAll("g").data(t);
            y.exit().remove();
            var e = y.enter().append("g").attr("class", "node").attr("transform", function (t) {
                return "x" in t && "y" in t ? "translate(" + t.x + ", " + t.y + ")" : ""
            }).on("mousedown.select-node", g).on("mouseenter.enter-node", w).on("mouseleave.leave-node", f).on("dblclick", D).call(d3.drag().on("start", S).on("drag", A).on("end", O));
            ...
            e.append("circle").attr("class", "foreground-circle").style("fill", "#1e98ce").style("cursor", "move").style("stroke-width", "3px").style("stroke", "#18769f").attr("flag","false");
            y = y.merge(e);
            y.selectAll("circle").attr("r", function (t) {
                if (!("size" in t)) {
                    t["size"] = R.node.node_size
                }
                return t.size * R.node.node_scale
            });
            N()
        }
        ...
        function N() {
            ...
            y.select("text").remove();
            y.filter(function (t) {
                return typeof t.label !== "undefined" ? true : false
            }).append("text").text(function (t) {
                return t.label
            }).attr("x", function (t) {
                return j(d3.select(this), t[e])
            }).attr("dy", ".35em").style("display", R.show_status.node_text === true ? "block" : "none").style("pointer-events", "none").style("fill", "white")
        }
        ...
        function d(t, e) {
            ...
            r.exit().remove();
            var i = r.enter().append("g").attr("class", "link");
            i.append("path").attr("class", "link-path").style("fill", "none").style("cursor", "pointer").style("stroke", "#6f7071");
            i.append("marker").attr("class", "link-marker").attr("markerUnits", "userSpaceOnUse").attr("viewBox", "0 -50 100 100").attr("refY", 0).attr("markerWidth", 12).attr("markerHeight", 12).attr("orient", "auto").append("path").attr("d", "M20,0 L0,-30 L90,0 L0,30 L20,0").style("fill", "#000000");
            i.append("text").attr("class", "link-text").attr("dx", function (t) {
                return E(t)
            }).attr("dy", 4).append("textPath").style("font-size", "10px").style("fill", "#000000").style("display", R.show_status.link_text === true ? "block" : "none");
            r = r.merge(i);
        ...
        function D(M) {
            b();
            var t = (M.size ? M.size : R.node.size) + 8;
            var e = t + 30;
            ...
            var N = "M" + t * n + ", -" + t * r + " " + "A" + t + ", " + t + " 0 0, 1 " + t * i + ", " + t * a + "L" + u + "," + c + " " + "A" + e + ", " + e + " 0 0, 0 " + e * L + ", -" + e * l + "L" + t * n + ",-" + t * r;
            ...
            j.append("path").attr("id", function (t, e) {
                ...
                if (t === 0) {
                    ...
                    J.on("mousemove.add-link", function () {
                        var t = v("#container");
                        var e = t.k;
                        n.attr("x2", (d3.event.x - q.x - t.x) / e).attr("y2", (d3.event.y - q.y - t.y) / e);
                        n.style("opacity", 1)
                    });
                    y.selectAll("circle").style("cursor", "pointer").on("click.add-link", function (t) {
                        n.remove();
                        J.on("mousemove.add-link", null);
                        y.selectAll("circle").style("cursor", "move").on("click.add-link", null);
                        _(e, t, "TEST")
                    })
                } else if (t === 2) {
                    B(M)
        ...
        function A(t) {
            ...
            if (X.length !== 0) {
                ...
                X.select("text").attr("dx", function (t) {
                    return E(t)
                })
            }
        }
        ...
        function C(t) {
            ...
            if (t.source === t.target) {
                var a = 30 / t.path_count;
                e = "M" + n + ", " + M + "A" + a + ", " + a + " 0 1,1 " + r + ", " + (i + 1)
        ...
        function m(t) {
            i = t;
            if (e === "force") {
                r.nodes(t.nodes).force("link").links(t.links)
            } else if (e === "radius") {
        ...
        function Q() {
            R.graph.calculating = true;
            d3.select("#network-status").style("animation", "calc ease 1s infinite");
            r.alpha(1).restart()
        }

        function b() {
            R.graph.calculating = false;
            d3.select("#network-status").style("animation", "none");
            r.stop()
        }

        function U(t) {
            R.graph.layout_mode = t
        }

        function P(t) {
            R.graph.analyse_mode = t
        }

        function Z(t, e, n) {
            b();
            t["x"] = arguments[1] ? e : q.width / 2;
            t["y"] = arguments[2] ? n : q.height / 2;
            i.nodes.push(t);
            m(i)
        }
        ...
        function B(e) {
            ...
            i.links = i.links.filter(function (t) {
        ...
        function H(n, t, e) {
            ...
            if (e === true) {
                r.force("link", d3.forceLink().id(function (t) {
                    return t.id
                }).strength(.005)).force("charge", d3.forceManyBody().strength(R.graph.repulsion)).force("link").links(t)
            }
            d(t, F, "0")
        }
    ...
    var J = null, V = null, F = null, X = [];
    var q = Object();

    function M(t) {
        J = d3.select("#" + (typeof t !== "undefined" ? t : "body"));
        J.on("click", function () {
            d3.selectAll(".menu").remove()
        });
        V = J.append("g").attr("id", "container");
        F = V.append("g").attr("id", "connect_layout");
        q = document.getElementById(t).getBoundingClientRect();
